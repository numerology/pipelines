          ...
          if task.arguments is not None:
            for argument in task.arguments.values():
              if isinstance(
                ...
                raise TypeError(
                    'Argument "{}" references non-existing input.'.format(
                        argument))
...
class ComponentReference(ModelBase):
  ...
  Contains information that can be used to locate and load a component by
  name, digest or URL
  """
...
class TaskSpec(ModelBase):
  ...
  Task is a "configured" component - a component supplied with arguments and
  other applied configuration changes.
  """
  _serialized_names = {
...
class GraphSpec(ModelBase):
  ...
  It represents a graph of component tasks connected to the upstream sources
  of data using the argument specifications. It also describes the sources of
  graph output values.
  """
  _serialized_names = {
