            ...
            for parameter_argument in parameter_arguments:
                ...
                if placeholder_type == "inputs":
                    ...
                    template_input_to_parent_dag_inputs.setdefault(
                        (task_template_name, task_input_name), set()
                    ).add((dag_template_name, dag_input_name))
...
def _replace_output_dir_and_run_id(
    ...
    if _components.OUTPUT_DIR_PLACEHOLDER in command_line:
        if not output_directory:
            raise ValueError(
                "output_directory of a pipeline must be specified "
            ...
            def reconcile_output_name(
                ...
                for cmd in command_lines:
                    ...
                    matched_output_name_pattern = re.match(
                        r"{{kfp\.input-output-name\.(?P<input_name>.*)}}", cmd
                    )
...
def _refactor_dag_template_uri_inputs(
    ...
    for task in dag_template["dag"].get("tasks", []):
        ...
        for artifact_arg in artifact_args:
            assert "name" in artifact_arg, "Illegal artifact format: %s" % artifact_arg
            if (template_name, artifact_arg["name"]) in refactored_inputs:
                ...
                assert artifact_arg.get("from", "").startswith(
                    "{{inputs."
                ) or artifact_arg.get("from", "").startswith("{{tasks."), (
                ...
                if arg_from.startswith("{{tasks."):
                    ...
                    input_matches = re.match(
                        r"{{inputs\.artifacts\.(?P<input_name>.*)}}", arg_from
                    )
